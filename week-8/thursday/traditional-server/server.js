...
const server = http.createServer((req, res) => {
  ...
  req.on("end", () => {
      ...
      const dogsList = dogs.map(dog => {
        return `
          <li>
            ...
            <a href="/dogs/${dog.dogId}/edit">
              <button>Edit</button>
            </a>
            <form action="/dogs/${dog.dogId}/delete" method="POST">
    ...
    // Phase 6: POST /dogs/:dogId/delete
    if (req.method === 'POST' && req.url.endsWith('/delete')) {
      const urlParts = req.url.split('/');
      if (urlParts.length === 4) {
        const dogId = urlParts[2];
        console.log(dogId);
        dogs = dogs.filter(dog => dog.dogId !== parseInt(dogId));
        console.log(dogs);
        res.statusCode = 302;
